...
-- Find the average weight of heroes.
SELECT ROUND(AVG(weight_kg)) AS avg_weight FROM marvels;

-- Create column indicating whether hero is above or below avg weight
SELECT name, weight_kg,
    CASE
        WHEN weight_kg > 170 THEN "Above average"
        WHEN weight_kg < 170 THEN "Below average"
    END AS weight_wrt_avg
FROM marvels;

-- Count how many heroes are in the designated categories above or below average weight
SELECT
    CASE
        WHEN weight_kg > 170 THEN "Above average"
        WHEN weight_kg < 170 THEN "Below average"
    END AS weight_wrt_avg, COUNT(*)
FROM marvels GROUP BY weight_wrt_avg;
    
-- Find the hometown/country with the greatest amount of superheroes
SELECT hometown, COUNT(*) AS hometown_count FROM marvels GROUP BY hometown ORDER BY hometown_count DESC;

-- Find the fastest female heroes
SELECT name, popularity, gender, speed FROM marvels WHERE gender = "Female" AND speed > 3;
