...
SELECT
    CONCAT(SUBSTRING(title, 1, 9), "...") AS shortened_title
FROM
    books;
...
SELECT
    CONCAT_WS(
        ...
        REVERSE(REPLACE(title, " ", "_"))
    ) AS replace_reversed_info
FROM
    books;
...
SELECT
    author_lname,
    CHAR_LENGTH(author_lname) AS lname_length
FROM
    books;
...
SELECT
    ...
    CONCAT_WS(",", LOWER(author_lname), UPPER(author_fname)) AS author,
...
SELECT
    DISTINCT CONCAT(title, " - ", released_year) AS book_summ,
    released_year
FROM
    books
ORDER BY
